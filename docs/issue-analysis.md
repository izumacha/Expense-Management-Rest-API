...
- `AuditLogReadmeDdlTest` — README に載せた手書き DDL が `AuditLog` のマッピング（列名・型・
  ...
  README 側の DDL は自前で読み解くため、**読めるのは列・主キー・識別列・インデックスまで**と範囲を決め、
  それ以外の記述（UNIQUE / CHECK 制約、`ALTER TABLE`、索引の並び順や演算子クラスなど）は
  **素通りさせず落とす**。読み飛ばすと、エンティティに無い一意制約を README に足しても通ってしまい、
  同じ行への 2 度目の記録が制約に弾かれる——監査の書き込みは fail-open、`validate` も制約は見ないため、
  どこにも合図が出ないまま監査証跡が欠ける。扱えない構文が必要になったら検証側を先に広げる。
