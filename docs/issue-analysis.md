...
**再評価後の重大度サマリ**: 当初の 12 件（「重大 3 件」「高 4 件」「中 4 件」「低 3 件」）は
**すべて解消済み**。ただし当初分析より後に追加した所見（A.1 専用の監査ログがない）は**未対応**である。
...
> 書き換えず保持し、当初 12 件（1.1〜2.6）については上表のステータス欄だけを更新する。
...
> 上表にも同じ ID で 1 行足す。当初 12 件の凍結を保ったまま、棚卸し表としての網羅性を維持するため。
