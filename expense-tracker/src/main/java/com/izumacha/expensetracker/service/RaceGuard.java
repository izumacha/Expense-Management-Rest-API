...
// 楽観ロックの行数不一致例外（UPDATE/DELETEの対象行自体が同時実行で消えた場合に発生する）
...
// (DataIntegrityViolationException)・対象行消失(OptimisticLockingFailureException)として検知し、
...
final class RaceGuard {
    ...
    // 保存/削除などの実処理を実行し、一意制約違反と対象行消失レースをそれぞれ変換する。
    ...
    static <T> T guarded(Supplier<T> action,
        ...
        try {
            ...
            // 対象行が同時実行で既に消えていた(影響行数0件)ケースを、呼び出し元が指定した
            // ドメイン例外へ変換して送出する
