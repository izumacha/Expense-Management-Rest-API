...
class AuditLogReadmeDdlTest {
    ...
    /**
     * 識別列の言い回し（{@link #IDENTITY_KEYWORD}）を取り除いた後に、単独で現れてよい修飾子。
     *
     * <p>{@link #SUPPORTED_COLUMN_MODIFIERS} には {@code generated by default as identity} を
     * 構成する語が含まれるため、そのまま 1 語ずつ照らすと {@code DEFAULT NULL} のような
     * <b>既定値の指定</b>まで「知っている語の並び」として通ってしまう。既定値は正本と
     * 突き合わせていないので、言い回しを先に取り除いてからこの集合で確かめる。
     */
    private static final Set<String> STANDALONE_COLUMN_MODIFIERS =
            Set.of("not", "null", "primary", "key");
    ...
    private static Schema parseReadmeSchema(String expectedTableName) {
        ...
        for (String rawStatement : block.split(";")) {
            ...
            // テーブル定義なら、名前を確かめてから括弧の内側を取り出して覚えておく
            if (statement.startsWith(CREATE_TABLE_KEYWORD)) {
                ...
                String tableName = statement.substring(CREATE_TABLE_KEYWORD.length(), tableOpen).trim();
                ...
                // テーブル名が正本と一致することを確かめる。
                // 【なぜ必要か】ブロックの選び方は「対象テーブルに言及している SQL ブロック」なので、
                // 索引定義がテーブル名に触れていれば、テーブル定義側の名前が違っていてもブロックは選ばれる。
                // ここで見ないと、`CREATE TABLE totally_wrong (...)` のまま緑になり、適用した本番は
                // 想定と違う名前のテーブルができて validate が起動を拒む——この検証が防ぐはずの失敗そのもの
                assertThat(tableName)
                        // 失敗時に何がずれているかと直し方を示す
                        .as("README の DDL のテーブル名が @Table の宣言と一致しません"
                                + "（テーブル名を変えたら README も同じ PR で直してください）")
                        // 正本のテーブル名と一致することを検証する
                        .isEqualTo(expectedTableName);
                // 同じブロックにテーブル定義が 2 つ以上あれば、後から来た方が黙って上書きしてしまう。
                // 上書きされた側は誰にも検証されないまま本番へ適用されうるので止める
                assertThat(tableBody)
                        // 失敗時に何が起きているかを示す
                        .as("README の SQL ブロックにテーブル定義が複数あります: %s", statement)
                        // まだ 1 つも読んでいないことを検証する
                        .isNull();
                // 括弧の内側を本体として覚えておく
        ...
        for (String rawItem : splitTopLevel(tableBody)) {
            ...
            // 識別列の指定はひとまとまりの言い回しなので、先に取り除いてから 1 語ずつ確かめる
            // （`default` を単語として許すと `DEFAULT NULL` のような既定値指定まで通ってしまう）
            String remainingModifiers = modifiers.replace(IDENTITY_KEYWORD, " ").trim();
            // 残った修飾子を 1 語ずつ確かめる
            for (String modifier : remainingModifiers.split("\\s+")) {
                // 空白だけなら確かめる語が無いので飛ばす
                if (modifier.isEmpty()) {
                    // 次の語へ進む
                    continue;
                ...
                if (!STANDALONE_COLUMN_MODIFIERS.contains(modifier)) {
                    // 何が扱えないかを示して停止する
                    rejectUnsupported("列 " + name + " に付いた指定「" + modifier + "」", item);
                }
            }
