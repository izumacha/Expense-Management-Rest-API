...
- 認証の監査記録に **Spring Security の認証イベント（`AuthenticationSuccessEvent` 等）を購読しない**。Bearer トークンを検証するリソースサーバも同じイベントに乗るため、購読すると (1) 通常の API 呼び出し 1 回ごとに「ログイン成功」が記録されて監査テーブルが膨れ、(2) トークン検証失敗では**トークン文字列そのもの**が actor として保存される（＝資格情報を追記専用テーブルに書き込む）。記録は `AuthTokenService` の中だけで行う（`AuthenticationAuditScopeTest` が過不足の両方を固定している）。
...
- `AuditLog` の列・インデックスを変えたら、**同じ PR で README「監査ログの確認」の手書き DDL も直す**（本番は `ddl-auto: validate` で手動適用するため）。ずれは `AuditLogReadmeDdlTest` が落とす。とくにインデックスは `validate` の検証対象外で、README への反映漏れは本番だけインデックスが欠けた状態を静かに作る。同テストが読めるのは列・主キー・識別列・インデックスまでで、それ以外の記述（UNIQUE / CHECK 制約、`ALTER TABLE`、索引の並び順など）は素通りさせず落とす（正本と突き合わせられないものを見逃すと監査記録が静かに欠けるため）。README の DDL は 1 箇所だけに置く（写しが増えると検証されない方が古くなる）。
