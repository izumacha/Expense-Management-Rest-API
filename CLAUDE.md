...
- `AuditLog` の列・インデックスを変えたら、**同じ PR で README「監査ログの確認」の手書き DDL も直す**（本番は `ddl-auto: validate` で手動適用するため）。ずれは `AuditLogReadmeDdlTest` が落とす。とくにインデックスは `validate` の検証対象外で、README への反映漏れは本番だけインデックスが欠けた状態を静かに作る。
