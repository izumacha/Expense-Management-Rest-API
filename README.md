...
アプリのログ（エラー・レート制限の警告など）は**コンテナの標準出力**に出ます。「誰がいつ何を変更したか」の追跡には、アプリログではなく DB の**監査ログテーブル**（`audit_logs`。次の「監査ログの確認」参照）を使ってください。
...
まず注意点として、**認証の成否はアプリログには残りません。** トークン発行の失敗は 401 を返すだけでログ出力を伴わず（`GlobalExceptionHandler#handleAuthenticationFailure`）、成功時も同様です。認証イベントは代わりに `audit_logs` テーブルへ記録されるので、`POST /api/auth/token` への総当たりはそちらで検知・立証します（次の「監査ログの確認」参照）。現在アプリログに出るのは主にサーバエラー（5xx）と、追跡クライアント数が上限に達したときのレート制限の警告です。
...
「誰が・いつ・どの行に・どんな操作をしたか」は DB の `audit_logs` テーブルに残ります。アプリログと違ってコンテナを消しても残り、DB バックアップ（次項）にもそのまま含まれます。

記録されるのは次の 2 種類です。

| 種類 | `entity_name` | `entity_id` | `action` |
|---|---|---|---|
| データ変更 | `Expense` / `Category` | 対象行の ID | `CREATE` / `UPDATE` / `DELETE` |
| 認証 | `Authentication` | （なし） | `LOGIN_SUCCESS` / `LOGIN_FAILURE` |

参照は DB へ直接接続して行います（監査ログ自体が機微な情報なので、参照 API は用意していません）。

```bash
# 直近 50 件を新しい順に見る
docker compose exec db psql -U expensetracker -d expensetracker \
  -c "SELECT occurred_at, actor, action, entity_name, entity_id FROM audit_logs ORDER BY occurred_at DESC LIMIT 50;"

# 特定の支出（ID=12）の変更履歴を追う
docker compose exec db psql -U expensetracker -d expensetracker \
  -c "SELECT occurred_at, actor, action FROM audit_logs WHERE entity_name='Expense' AND entity_id='12' ORDER BY occurred_at;"

# 直近 1 時間のログイン失敗を数える（総当たりの検知）
docker compose exec db psql -U expensetracker -d expensetracker \
  -c "SELECT actor, count(*) FROM audit_logs WHERE action='LOGIN_FAILURE' AND occurred_at > now() - interval '1 hour' GROUP BY actor ORDER BY count DESC;"
```

**運用上の注意**

- **追記のみ・自動削除なし**: 行は増え続けます。長期運用ではディスクを見ながら、古い行を業務要件に沿った保存期間で手動削除してください（監査記録なので、アプリからは消せないようにしてあります）。
- **記録の欠落は起こりうる**: 監査の書き込みに失敗しても API は止めません（DB 断で家計簿が使えなくなる方が困るため）。失敗したときはアプリログに `監査ログの記録に失敗しました` という WARN が出るので、監視するならこの文言を拾ってください。
- **本番でテーブルを作るには**: 既定のスキーマ方針は `validate`（DDL を自動発行しない）です。既存の本番 DB へは次の DDL を手動で適用してください（`docker compose` のローカル環境と Testcontainers は自動で作成されるため不要です）。
...
- **監査ログの範囲**: 監査テーブル（`audit_logs`）は導入済みで、支出・カテゴリの作成/更新/削除と認証イベント（トークン発行の成功/失敗）を記録します（上の「監査ログの確認」参照）。ただし範囲は限定的です。**(1) 値の差分は残しません**（「支出 12 が更新された」までで、金額がいくらからいくらに変わったかは記録しません。家計情報を監査テーブルへ複製しないための意図的な設計です）。**(2) 記録は fail-open** で、書き込みに失敗しても API は成功します（欠落は WARN ログでのみ分かります）。**(3) 参照 API はありません**（DB へ直接接続して読みます）。**(4) 保存期間の自動管理はありません**（行は増え続けます）。詳細と判断の経緯は `docs/issue-analysis.md` の追加所見 A.1 を参照してください。
