#!/usr/bin/env bash
# PostgreSQL（docker-compose.yml の db サービス）向けの自動バックアップスクリプト。
# helpdesk-hub / incident-insight の scripts/backup-db.sh を移植したもの (docs/backup.md 参照)。
#
# 何をするか:
#   1. DATABASE_URL が指す PostgreSQL を pg_dump の custom 形式 (-Fc, 圧縮込み) でダンプする。
#   2. ダンプを BACKUP_DIR にタイムスタンプ付きファイル名で保存する。
#   3. BACKUP_RETENTION_DAYS より古い自動取得分のダンプを削除する (世代管理 / ディスク枯渇防止)。
#
# 設計方針:
#   - fail-closed: 失敗したら途中で安全に停止する (set -euo pipefail)。
#   - 秘密情報 (パスワード) はログに出さない (DATABASE_URL をそのまま echo しない)。
#   - ダンプには支出データ (金額・日付・メモ等の個人の家計情報) が含まれるため、保存先の
#     アクセス権限を必ず絞ること (docs/backup.md のセキュリティ注意を参照)。
#   - 自動取得分は auto_expensetracker_ プレフィックスを付け、README の手動バックアップ
#     (expensetracker_*) と名前空間を分ける。世代削除も自動取得分だけを対象にするため、
#     アップグレード前に取った手動スナップショットを誤って消さない (README の運用手順と同じ方針)。
...
# では 0644 = 全ユーザーが読めるファイルになる。ダンプの中身は支出データそのものなので、
# バックアップ用ホストに一般ユーザーのログインがあると平文で読めてしまう。
...
# バックアップの保存先ディレクトリ (未設定なら README の手動手順と同じ ./backup を使う)
BACKUP_DIR="${BACKUP_DIR:-./backup}"
# 何日分のバックアップを残すか (これより古い自動取得分のダンプは削除する。既定 14 日)
BACKUP_RETENTION_DAYS="${BACKUP_RETENTION_DAYS:-14}"
# ダンプ対象 DB の接続文字列 (アプリと同じ接続先を再利用する)
DATABASE_URL="${DATABASE_URL:-}"
# 自動取得分のファイル名プレフィックス (手動バックアップ expensetracker_* と名前空間を分ける)
DUMP_PREFIX="auto_expensetracker_"
...
環境変数:
  ...
  BACKUP_DIR              バックアップ保存先 (既定: ./backup)
  BACKUP_RETENTION_DAYS   保持日数。これより古い自動取得分 (auto_expensetracker_*.dump) は削除 (既定: 14)
...
# 実行ホストのローカルタイムゾーンで命名されるため、GitHub Actions では backup.yml が
# TZ=Asia/Tokyo を設定して JST 03:00 の定期実行が「その日の日付」になるようにしている。
...
# (空/不完全なダンプを「成功した世代」と誤認させないための後始末。README の一時ファイル方式と同じ意図)。
...
# ───────────────────────────────────────────────────────────────────────────
# 世代管理 (保持日数より古い自動取得分のダンプを削除する)
# ───────────────────────────────────────────────────────────────────────────

# 削除対象 (BACKUP_RETENTION_DAYS 日より古い自動取得分 *.dump) を find で列挙して数える。
# -mtime +N は「N 日より古い」を意味するため、保持日数からそのまま指定できる。
# プレフィックスを自動取得分に限定し、手動バックアップ (expensetracker_*) は削除しない。
PRUNED=0
...
while IFS= read -r -d '' old_dump; do
    ...
    # 削除カウンタを 1 増やす
    PRUNED=$((PRUNED + 1))
done < <(find "$BACKUP_DIR" -maxdepth 1 -type f -name "${DUMP_PREFIX}*.dump" -mtime "+${BACKUP_RETENTION_DAYS}" -print0)
