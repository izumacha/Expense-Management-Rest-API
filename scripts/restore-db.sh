...
# helpdesk-hub / incident-insight の scripts/restore-db.sh を移植したもの (docs/backup.md 参照)。
#
# 何をするか:
...
# 引数はダンプファイル 1 つだけ。余分な引数はタイプミス (オプションの綴り間違いなど) の
# 可能性が高く、黙って無視すると「意図と違うファイルを本番 DB へ流し込む」事故になりうる。
# backup-db.sh が未知の引数をエラーにしているのと同じ方針で、ここでも fail-closed にする。
if [ "$#" -gt 1 ]; then
    # 使い方を表示してから、余分な引数を示して停止する
    usage
    die "引数が多すぎます (ダンプファイルは 1 つだけ指定してください): $*"
fi
